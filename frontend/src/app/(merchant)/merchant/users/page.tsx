...
import { Plus, Edit, UserPlus, Shield, Mail } from 'lucide-react';
...
export default function MerchantUsersPage() {
  ...
  const { data: users, isLoading } = useQuery({
    queryKey: ['users', currentUser?.merchantId],
    queryFn: async () => {
      const res = await apiClient.get(`/users?merchantId=${currentUser?.merchantId}`);
  ...
  const { data: roles } = useQuery({
    queryKey: ['roles', 'MERCHANT'],
    queryFn: async () => {
      const res = await apiClient.get('/roles?type=MERCHANT');
      return res.data.data || [];
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Merchant Users</h1>
          <p className="text-muted-foreground">Create and manage merchant users</p>
        </div>
          ...
          >
            ...
            <DialogContent className="max-w-lg">
              <DialogHeader>
                ...
                <DialogDescription>
                  Create a new merchant user. A temporary password will be generated and sent via email.
                </DialogDescription>
              </DialogHeader>
                  ...
                  <div className="border rounded-md p-4 max-h-48 overflow-y-auto space-y-2">
                    ...
                    {(!roles || roles.length === 0) && (
                      <p className="text-sm text-muted-foreground">
                        No merchant roles available. Platform owner must create roles first.
                      </p>
                    )}
                  </div>
                </div>
